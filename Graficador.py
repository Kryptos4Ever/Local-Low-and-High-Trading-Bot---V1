"""
Graficador — Análisis de Estrategias BTC/USDT
═══════════════════════════════════════════════
Visualiza cualquier estrategia del sistema a partir del JSON
producido por backtest_irreal.py o backtest_compuesto.py.

Cambios respecto al original
──────────────────────────────
  · Importa desde config_local (nuevo esquema: DB_TABLE, MAX_POSICIONES)
  · analisis_consola() muestra parámetros genéricos del JSON en lugar de
    campos hardcodeados — funciona con irreal, compuesto y futuras estrategias
  · cargar_precios() usa DB_TABLE explícito
  · Normalización de datetime: maneja ISO 8601 con Z y sin timezone
  · Sin referencias a versión en nombre ni contenido
...
# ── Importar config_local ─────────────────────────────────────────────────────
try:
    from config_local import (
        DB_PATH, DB_TABLE, RESULTS_JSON,
        FECHA_INICIO, FECHA_FIN,
        SALDO_USDT_INICIAL, COMMISSION_PCT,
        DARK_MODE, OUTPUT_PNG, DPI,
    )
except ImportError as e:
    print(f"✗ No se encontró config_local.py: {e}")
    print("  Asegurate de ejecutar desde la raíz del proyecto.")
    raise
...
def _norm_dt_index(idx: pd.DatetimeIndex) -> pd.DatetimeIndex:
    """Normaliza a naive UTC ms — elimina timezone y unifica resolución."""
    if hasattr(idx, "tz") and idx.tz is not None:
        idx = idx.tz_localize(None)
    return idx.astype("datetime64[ms]")


def interpolar_serie(
    ...
    """
    Interpola (ffill) una columna del DataFrame de trades sobre el índice
    de precios. Normaliza timezone y resolución antes de reindexar para
    evitar TypeError entre datetime64[ms] y datetime64[us, UTC].
    """
    if col not in trade_df.columns:
        return pd.Series(fill_value, index=price_index)
    s = trade_df[col].copy()
    s = s[~s.index.duplicated(keep="last")]
    s.index      = _norm_dt_index(s.index)
    pidx_norm    = _norm_dt_index(price_index)
    s            = s.reindex(pidx_norm, method="ffill").fillna(fill_value)
    s.index      = price_index
    return s


def separar_trades(df: pd.DataFrame):
    ...
    return (
        ejecutados[ejecutados["type"] == "BUY"],
...
class Graficador:
    ...
    def __init__(self, dark_mode: bool = DARK_MODE) -> None:
    ...
    def cargar_precios(
        ...
        try:
            ...
            df   = pd.read_sql(
                f"SELECT * FROM {table} ORDER BY timestamp ASC", conn
            )
            conn.close()
    ...
    def cargar_resultados(self, json_path: str = RESULTS_JSON) -> bool:
        try:
            with open(json_path, encoding="utf-8") as f:
                self.results_data = json.load(f)
            trades = self.results_data.get("trade_history", [])
            df = pd.DataFrame(trades)
            # ISO 8601 con Z → naive UTC ms (sin timezone, sin microsegundos)
            df["datetime"] = (
            ...
            self.trade_data = df
            print(f"✓ Resultados: {len(df):,} registros desde '{json_path}'")
            return True
        except Exception as e:
    ...
    def _rango(self):
        """Retorna (fi, ff) como Timestamps naive."""
        fi = pd.to_datetime(FECHA_INICIO) if FECHA_INICIO \
             else self.price_data["datetime"].min()
        ff = pd.to_datetime(FECHA_FIN)    if FECHA_FIN    \
             else self.price_data["datetime"].max()
        if hasattr(fi, "tzinfo") and fi.tzinfo is not None:
            fi = fi.tz_localize(None)
        if hasattr(ff, "tzinfo") and ff.tzinfo is not None:
            ff = ff.tz_localize(None)
        return fi, ff

    def _price_cont(self) -> pd.DataFrame:
        fi, ff = self._rango()
        ...
        if "ignorado" in self.trade_data.columns:
            td = self.trade_data[
                ~self.trade_data["ignorado"].fillna(False).astype(bool)
            ].copy()
        else:
        ...
        for col, fill in [
            ("usdt_balance",      SALDO_USDT_INICIAL),
            ("btc_balance",       0.0),
        ...
        if "precio_promedio_posiciones" in td.columns:
            ...
            pp.index   = _norm_dt_index(pp.index)
            pp_r       = pp.reindex(_norm_dt_index(price.index), method="ffill")
            pp_r.index = price.index
            price["precio_promedio"] = pp_r
        else:
        ...
        price["bh_value"]        = (SALDO_USDT_INICIAL / price["close"].iloc[0]) \
                                   * price["close"]
        price["port_ret"]        = price["portfolio_value"].pct_change().fillna(0)
        return price

    def _filtrar_trades(self) -> pd.DataFrame:
        fi, ff = self._rango()
        m = (
            (self.trade_data["datetime"] >= fi) &
            (self.trade_data["datetime"] <= ff)
        )
        return self.trade_data[m].copy()

    # ── Análisis en consola ───────────────────────────────────────

    def analisis_consola(self) -> None:
        ...
        # Campos internos sin valor informativo para el usuario
        SKIP = {
            "estrategia", "rsi_length", "ath_caida_maxima",
            "atl_subida_maxima", "factor_caida", "factor_subida",
            "N", "guardia_compra", "guardia_venta",
        }
        for k, v in par.items():
            if k in SKIP or v in ("N/A", None):
                continue
        ...
        ganancias = [
            t for t in (
                v_ok["ganancia_usdt"].dropna().tolist()
                if "ganancia_usdt" in v_ok.columns else []
            ) if t is not None
        ]
        if ganancias:
        ...
        print(f"  Período              : {str(fi_dt)[:10]}  →  "
              f"{str(ff_dt)[:10]}  ({dur}d)")
        print(f"  Ejecutados           : {tot}  "
              f"(compras: {len(c_ok)}  ventas: {len(v_ok)})")
        print(f"  Ignorados            : {len(c_ign) + len(v_ign)}")
        if dur > 0 and tot > 0:
            print(f"  Frecuencia           : "
                  f"{tot / (dur / 30):.1f} trades/mes  |  "
                  f"{dur / tot:.1f}d entre trades")
        print(sep)

    # ── Gráfico principal ─────────────────────────────────────────

    def crear_grafico(self, output: str = OUTPUT_PNG) -> str:
        ...
        def panel_title(ax, texto: str, fontsize: float = 9.5) -> None:
            ax.text(0.5, 1.003, texto, transform=ax.transAxes,
                    ha="center", va="bottom", fontsize=fontsize,
                    color=C["text"], clip_on=False, zorder=20)

        def right_box(ax, texto: str, y: float = 0.97) -> None:
        ...
        fig.suptitle(
            ...
            f"Portfolio: ${pfin:,.0f}   "
            f"Compras: {s.get('total_compras', '-')}   "
            f"Ventas: {s.get('total_ventas', '-')}   "
        ...
        if len(c_ok):
            ax.scatter(c_ok["datetime"], c_ok["price"], c=C["buy_ok"],
                       ...
                       label=f"Compras ({len(c_ok)})")
        if len(v_ok):
            ax.scatter(v_ok["datetime"], v_ok["price"], c=C["sell_ok"],
                       s=20, alpha=0.8, zorder=5,
                       label=f"Ventas ({len(v_ok)})")
        ...
        if pp_fin and pp_fin > 0:
            ...
            ax.annotate(f"PP: ${pp_fin:,.0f}",
                        xy=(pc.index[-1], pp_fin),
        ...
        # ── Panel 3: Balance BTC ──────────────────────────────────
        ax = axes[ax_idx]; ax_idx += 1
        ax.plot(pc.index, pc["btc_en_posiciones"], color="#60a5fa",
                lw=1.1, label="BTC en posiciones")
        ax.plot(pc.index, pc["btc_balance"],       color="#c084fc",
                lw=1.1, label="BTC libre (acum.)")
        ax.plot(pc.index, pc["btc_total"],         color=C["btc_line"],
                lw=1.8, label="BTC total")
        ...
        right_box(ax,
                  ...
                  f"Total: {btc_pos + btc_libre:.6f} BTC",
                  y=0.38)
        ...
        ax = axes[ax_idx]; ax_idx += 1
        ax.plot(pc.index, pc["positions_count"], color="#a78bfa",
                lw=1.4, label="Posiciones abiertas")
        ...
        ax.axhline(SALDO_USDT_INICIAL, color=C["grid"], ls="--", lw=0.8,
                   alpha=0.7, label=f"Capital inicial (${SALDO_USDT_INICIAL:,})")
        ...
        for i, ax in enumerate(axes):
            ...
            plt.setp(ax.get_xticklabels(), color=C["text"], fontsize=8,
        ...
        try:
            ...
            if "qt"  in backend: manager.window.showMaximized()
            elif "tk" in backend:
                try:    manager.window.state("zoomed")
                except Exception: pass
                try:    manager.window.attributes("-zoomed", True)
                except Exception: pass
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Graficador — Estrategias BTC/USDT"
    )
    parser.add_argument("--json",  default=RESULTS_JSON, help="JSON de resultados")
    parser.add_argument("--db",    default=DB_PATH,       help="Base de datos SQLite")
    parser.add_argument("--table", default=DB_TABLE,      help="Tabla en la DB")
    parser.add_argument("--dark",  action="store_true",   default=DARK_MODE)
    ...
    if not g.cargar_precios(args.db, args.table):
        return
