...
def render_html(st: dict) -> None:
    ...
    sig_cls = ("buy" if st["sig"]=="BUY" else
               "sell" if st["sig"]=="SELL" else "hold")
...
<title>Live Dashboard — Local Reversal</title>
<style>
...
<div class="hdr">
  <div><h1>Local Reversal — Live Testnet</h1>
    <div class="ts" style="text-align:left;margin-top:4px">
      THR_B={THR_B} · THR_T={THR_T} · Max pos={MAX_POSICIONES} · {SYMBOL}
    </div>
  </div>
  <div class="ts">Actualizado: {now_s}<br>
    <span style="color:#475569">Auto-refresca cada {DASHBOARD_REFRESH}s</span>
  </div>
</div>

<div class="grid">
  <div class="card">
    <div class="lbl">Portfolio total</div>
    <div class="val">${st['portfolio']:,.2f}</div>
    <div class="sub">Inicial: ${st['usdt_ini']:,.2f}</div>
  </div>
  <div class="card">
    <div class="lbl">P&amp;L sesión</div>
    <div class="val pnl">{'+' if pnl_p>=0 else ''}{pnl_p:.2f}%</div>
    <div class="sub pnl">{'+' if pnl_u>=0 else ''}${pnl_u:,.2f} USDT</div>
  </div>
  <div class="card">
    <div class="lbl">USDT libre</div>
    <div class="val">${st['usdt_lib']:,.2f}</div>
    <div class="sub">Slot: ${st['slot']:,.2f}</div>
  </div>
  <div class="card">
    <div class="lbl">Posiciones</div>
    <div class="val">{st['n_pos']} / {MAX_POSICIONES}</div>
    <div class="sub">BTC @ ${px:,.2f}</div>
  </div>
  <div class="card">
    <div class="lbl">Operaciones</div>
    <div class="val">{st['nb']+st['ns']}</div>
    <div class="sub">Buy {st['nb']} · Sell {st['ns']} · Ign {st['ni']}</div>
  </div>
  <div class="card">
    <div class="lbl">Próxima vela en</div>
    <div class="cd">{cd}</div>
    <div class="sub">Último tick: {ts_s}</div>
  </div>
</div>
...
<div class="sec">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
    <span>Señal emitida:</span>
    <span class="badge {sig_cls}">{st['sig']}</span>
    <span style="color:#64748b;font-size:11px">Close: ${px:,.2f}</span>
  </div>
  <div class="bar-row">
    <span class="bar-lbl">prob_bottom</span>
    <div class="bar-trk">
      <div class="bar-fil" style="width:{pb_w};background:{pb_col}"></div></div>
    <span class="bar-val" style="color:{pb_col}">{pb:.3f}</span>
  </div>
  <div style="font-size:10px;color:#475569;margin:0 0 6px 96px">umbral BUY = {THR_B}</div>
  <div class="bar-row">
    <span class="bar-lbl">prob_top</span>
    <div class="bar-trk">
      <div class="bar-fil" style="width:{pt_w};background:{pt_col}"></div></div>
    <span class="bar-val" style="color:{pt_col}">{pt:.3f}</span>
  </div>
  <div style="font-size:10px;color:#475569;margin-top:2px;margin-left:96px">
    umbral SELL = {THR_T}</div>
</div>

<h2>Posiciones abiertas ({st['n_pos']})</h2>
<div class="sec">{pos_html}</div>

<h2>Últimas operaciones (buy={st['nb']} sell={st['ns']} ign={st['ni']})</h2>
<div class="sec">
  <table><thead><tr>
...
def _build_state(wallet, nb, ns, ni, candle, pb, pt, sig, tlog, usdt_ini) -> dict:
